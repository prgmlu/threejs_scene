...
const DeviceOrientationControls = function (
	...
	const onTouchMoveEvent = (event) => {
		...
		if (event.touches.length === 1) {
			...
			const nextRotX =
				rotX +
				THREE.Math.degToRad((tempY - event.touches[0].pageY) / 4);
	...
	const calculateAvailableDragSpace = (xAxisRoation) => {
		...
		if (THREE.Math.radToDeg(availablePositiveXSpace) + xAxisRoation < 0) {
			...
			upwardsDragSpace =
				THREE.Math.radToDeg(availablePositiveXSpace) + xAxisRoation;
		}
		...
		if (THREE.Math.radToDeg(availableNegativeXSpace) + xAxisRoation > 0) {
			...
			downwardsDragSpace =
				THREE.Math.radToDeg(availableNegativeXSpace) + xAxisRoation;
		}
	...
	const withinXBounds = (touchXRotation, direction) => {
		...
		const [upperXSpace, lowerXSpace] =
			calculateAvailableDragSpace(touchXRotation);
	...
	this.setupControlsFromConfig = function (orbitControlsConfig) {
		upperXBound = THREE.Math.degToRad(
			'max_vertical_angle' in orbitControlsConfig
				? orbitControlsConfig.max_vertical_angle
				: MAX_POLAR_ANGLE,
		);
		lowerXBound = THREE.Math.degToRad(
	...
	this.update = function (ignoreUpdate) {
		...
		const alpha = scope.deviceOrientation.alpha
			? THREE.Math.degToRad(
					scope.deviceOrientation.alpha + scope.offset,
...
// * IMPORTANT: modified from original
DeviceOrientationControls.prototype = Object.create(
