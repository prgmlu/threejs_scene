...
export const dressUpFromString = (model, outfitString) => {
    ...
    if (outfit.hairColor){
        hairMeshNames.forEach((hairMeshName) => {
            let hairMesh = model.getObjectByName(hairMeshName);
            if (hairMesh){
                hairMesh.material.color.set(outfit.hairColor);
            }
        });
    }

    if (outfit.eyeColor){
        eyeMeshNames.forEach((eyeMeshName) => {
            let eyeMesh = model.getObjectByName(eyeMeshName);
            if (eyeMesh){
                eyeMesh.material.color.set(outfit.eyeColor);
            }
        });
    }
    ...
    if(outfit.skinTone && (outfit.makeup || outfit.makeup == 0)){
            let url = `https://cdn.obsess-vr.com/realtime3d/makeup-skintones/Sk${outfit.skinTone}_FemaleAvatar${
                outfit.makeup+1
            }_D.jpg`;
            console.log(url);
            ...
            let url = `https://cdn.obsess-vr.com/realtime3d/skintones_new/Sk${outfit.skinTone}_FemaleAvatar_D.jpg`;
            console.log(url);
            let mesh = model.getObjectByName("FemaleAvatar_Head");
            setMeshTextureImage(mesh, url);
        }
        else if(outfit.makeup){
            let url = `https://cdn.obsess-vr.com/realtime3d/makeup-skintones/Sk1_FemaleAvatar${
                outfit.makeup+1
            }_D.jpg`;
            console.log(url);
