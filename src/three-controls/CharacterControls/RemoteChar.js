...
let charTypeMap = {
    "female":{
        url:"https://cdn.obsess-vr.com/realtime3d/BaseFemaleAvatar_Ver15.glb",
...
export default class RemoteChar{
    ...
    playTalkingAnimation(){
        this.talkingAction.setLoop(THREE.LoopOnce);
        this.talkingAction.reset().play();
    }
    ...
    setupAnimations(){
        ...
        this.idleAction = this.mixer.clipAction( this.animations.filter((i)=>i.name==ANIMATION_NAMES.idle)[0] );
        this.walkingAction = this.mixer.clipAction( this.animations.filter((i)=>i.name==ANIMATION_NAMES.walk)[0] );
        this.wavingAction = this.mixer.clipAction( this.animations.filter((i)=>i.name==ANIMATION_NAMES.wave)[0] );
        this.talkingAction = this.mixer.clipAction( this.animations.filter((i)=>i.name==ANIMATION_NAMES.talk)[0] );
        this.currentAction = this.idleAction;
