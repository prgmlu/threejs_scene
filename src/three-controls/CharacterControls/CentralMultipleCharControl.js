...
export default class CentralMultipleCharControls{
    constructor(mainCharControlsObj, otherChars,localAvatarNameRef, femaleLocalAvatarOutfitStringRef,maleLocalAvatarOutfitStringRef, visibleGenderRef,toAddObjsRef, stopAvatarAnimationLoopRef, scene, camera){
        ...
        if(USE_TOOLTIP){
            window.addEventListener('resize',()=>{
                ...
                // this.labelRenderer && this.labelRenderer.setSize(window.mainRenderer.wid, window.innerHeight);
                this.labelRenderer && this.labelRenderer.setSize(window.mainRenderer.domElement.clientWidth, window.mainRenderer.domElement.clientHeight);
            })
    ...
    update = () => {
        ...
        effectCanvas.remove();
        cancelAnimationFrame(window.vidId);
