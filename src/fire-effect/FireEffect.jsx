...
import {createDatGui} from './PositionHelper';
...
THREE.FireShader = {
    ...
    uniforms: {
        ...
        "noiseScale"    : { type : "v4",    value : new THREE.Vector4(1, 2, 1, 0.7) },
        "magnitude"     : { type : "f",     value : 1.2 },
        "lacunarity"    : { type : "f",     value : 2.4 },
...
const FIRE_TEXTURE_URL =
	'https://cdn.obsess-vr.com/bravo/Fire_blue2.png'
...
export default class FireEffect extends Component {
    ...
    constructor(props) {
        ...
        this.magnitude = props.magnitude || 1.2;
        this.lacunarity = props.lacunarity || 2.4;
        this.gain = props.gain || 0.4;
        this.speed = props.speed || 1;


        this.fireTextureSrc  = props.fireTextureSrc || FIRE_TEXTURE_URL;
    }

    componentDidMount() {
        let loader = new THREE.TextureLoader();
        let fireTex = loader.load(this.fireTextureSrc);
            ...
            fireMaterial.uniforms.magnitude.value = this.magnitude;
            fireMaterial.uniforms.lacunarity.value = this.lacunarity;
            fireMaterial.uniforms.gain.value = this.gain;
            
            this.fireMaterial= fireMaterial;

            let fire = new THREE.Mesh(new THREE.BoxGeometry( 1.0, 1.0, 1.0 ), fireMaterial );
        ...
        this.clock.start();
        // this.renderUUID = this.threeJSWorldContext.addToRenderLoop();
        // this.threeJSWorldContext.request3DSceneRender();
        this.animate();
    }
    ...
    updateFire(time){
            ...
            invModelMatrix.copy( this.fire.matrixWorld ).invert()

            if( time !== undefined ) {
                this.fire.material.uniforms.time.value = time;
            }

            this.fire.material.uniforms.invModelMatrix.value = invModelMatrix;
            this.fire.material.uniforms.scale.value = this.scale;
    }

    animate(){
        requestAnimationFrame(this.animate.bind(this));
        const elapsedTime = this.clock.getElapsedTime();
        this.updateFire(elapsedTime*this.speed);
    }
