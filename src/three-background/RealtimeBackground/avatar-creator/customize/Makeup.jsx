...
import roughnessImg from './Sk_FemaleAvatar_R_v002.png';
...
let setMakeupFromTexture = (index, currentAvatar, selectedSkintone) => {
	let ind = index + 1;
	let url = `https://cdn.obsess-vr.com/realtime3d/makeup-skintones/Sk${selectedSkintone}_FemaleAvatar${ind}_D.jpg`;
	console.log(url);
	// swap the texture of the mesh
	let bodyMeshs = [
		window.femaleModel.getObjectByName('FemaleAvatar_Head'),
		window.maleModel.getObjectByName('FemaleAvatar_Head'),
	]
	// let mesh = currentAvatar.getObjectByName('FemaleAvatar_Head');

	bodyMeshs.forEach((mesh) => {
		setMeshTextureImage(mesh, url);
	});

	// addRoughness(currentAvatar);
	// addMetalness(currentAvatar);
	addRoughness(window.maleModel);
	addMetalness(window.maleModel);
	
	addRoughness(window.femaleModel);
	addMetalness(window.femaleModel);
	// return;
};

let addRoughness = (currentAvatar) => {
	let roughnessTextureUrl = "https://cdn.obsess-vr.com/realtime3d/makeup-skintones/Sk_FemaleAvatar_R.png";
	new THREE.TextureLoader().load(roughnessTextureUrl, (roughnessTexture) => {
...
let addMetalness = (currentAvatar) => {
	let metalnessTextureUrl = "https://cdn.obsess-vr.com/realtime3d/makeup-skintones/Sk_FemaleAvatar_M.png";
	new THREE.TextureLoader().load(metalnessTextureUrl, (metalnessTexture) => {

		let mesh = currentAvatar.getObjectByName('FemaleAvatar_Head');
		mesh.material.metalness = 1;
		mesh.material.metalnessMap = metalnessTexture;
		mesh.material.metalnessMap.flipY=false
		mesh.material.metalnessMap.needsUpdate = true;
		mesh.material.needsUpdate = true;
