...
const ColorPicker = ({ selectedColor,tabName, handlePicker, handleClose, avatar,outfitString, setHairColor, setEyeColor }) => {
								...
								{group.map((color, index) => {
									return (
										<button
											...
											onClick={() => {
												let parsed = JSON.parse(outfitString.current);
												parsed.hairColor=color;

												dressUpFromString(window.maleModel,JSON.stringify(parsed));
												dressUpFromString(window.femaleModel,JSON.stringify(parsed));
												outfitString.current = JSON.stringify(parsed);
												setHairColor(color);
												handlePicker(color);
								...
								{group.map((color, index) => {
									return (
										<button
											onClick={() =>{
												let parsed = JSON.parse(outfitString.current);
												parsed.eyeColor=color;
												// dressUpFromString(avatar,JSON.stringify(parsed));
